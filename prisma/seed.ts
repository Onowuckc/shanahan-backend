...
const ROLE_ACCOUNTS: {
  ...
  {
    email: process.env.SEED_LECTURER_EMAIL || 'lecturer@shanahanuni.edu.ng',
    username: process.env.SEED_LECTURER_STAFF_ID || 'SU/LEC/001',
    role: Role.LECTURER,
    firstName: 'Demo',
    lastName: 'Lecturer',
    phoneNumber: '08000000011',
  },
];
...
async function main() {
  ...
  // ─── 8. SEED DEMO STUDENT & APPLICANT FOR TESTING ─────────────────────────
  console.log('\n👤 Seeding demo student and applicant...');
  const compScienceProg = await prisma.program.findFirst({
    where: { name: { contains: 'Computer Science' } }
  });
  const compScienceDept = await prisma.department.findFirst({
    where: { name: { contains: 'Computer Science' } }
  });

  if (compScienceProg && compScienceDept) {
    const passwordHash = bcrypt.hashSync(defaultPassword, 10);
    
    // Seed Student
    const studentUser = await prisma.user.upsert({
      where: { email: 'student@shanahanuni.edu.ng' },
      update: {},
      create: {
        email: 'student@shanahanuni.edu.ng',
        username: 'SU/CMP/26/1001',
        password: passwordHash,
        role: Role.STUDENT,
        isEmailVerified: true,
        isClaimed: true,
    ...
    const johnDoe = await prisma.studentProfile.upsert({
      where: { userId: studentUser.id },
      update: {},
      create: {
        userId: studentUser.id,
        matricNumber: 'SU/CMP/26/1001',
        firstName: 'John',
        lastName: 'Doe',
        gender: 'MALE',
        dateOfBirth: new Date('2005-05-15'),
        level: 100,
        departmentId: compScienceDept.id,
        programId: compScienceProg.id,
      }
    });

    // Seed Applicant
    const applicantUser = await prisma.user.upsert({
      where: { email: 'applicant@shanahanuni.edu.ng' },
      update: {},
      create: {
        email: 'applicant@shanahanuni.edu.ng',
        username: 'SU/APP/26/1001',
        password: passwordHash,
        role: Role.APPLICANT,
        isEmailVerified: true,
        isClaimed: true,
    ...
    console.log('  ✅  Demo student and applicant seeded successfully');

    // Create payment invoice for student
    const tuitionCategory = await prisma.feeCategory.findFirst({
    ...
    if (tuitionCategory && activeSession && activeSession.semesters.length > 0) {
      const activeSemester = activeSession.semesters.find(s => s.isCurrent) || activeSession.semesters[0];
      
      // Create Fee Structure using findFirst & create to avoid composite unique input type issues
      let existingFeeStruct = await prisma.feeStructure.findFirst({
        where: {
      ...
      if (!existingFeeStruct) {
        existingFeeStruct = await prisma.feeStructure.create({
          data: {
            feeCategoryId: tuitionCategory.id,
            sessionId: activeSession.id,
            semesterId: activeSemester.id,
      ...
      // Create Payment Invoice for John Doe
      await prisma.payment.upsert({
        where: {
          id: 'demo-student-payment-invoice-1'
        },
        update: {},
        create: {
          id: 'demo-student-payment-invoice-1',
      ...
      console.log('  ✅  Demo student tuition invoice of N150,000 seeded');

      // ─── SEED COURSES AND PREREQUISITES ────────────────────────────────────
      console.log('\n📖 Seeding demo courses and prerequisites...');
      const cmp101 = await prisma.course.upsert({
        where: { code: 'CMP 101' },
        update: {},
        create: {
          code: 'CMP 101',
      ...
      const cmp102 = await prisma.course.upsert({
        where: { code: 'CMP 102' },
        update: {},
        create: {
          code: 'CMP 102',
          title: 'Algorithmic & Structured Programming',
          creditUnits: 4,
          departmentId: compScienceDept.id
        }
      });
      ...
      console.log('  ✅  Set CMP 101 as a prerequisite for CMP 102');

      // ─── SEED DYNAMIC MAX UNITS FOR SOFTWARE ENGINEERING ───────────────────
      const targetSemester = activeSession.semesters.find(s => s.isCurrent) || activeSession.semesters[0];
      const softwareEngDept = await prisma.department.findUnique({
        where: { code: '310' }
      });
      if (softwareEngDept) {
        await prisma.departmentMaxUnits.upsert({
          where: {
        ...
        console.log(`  ✅  Dynamic max units for Software Engineering (310) set to 21 for ${targetSemester.name} semester`);
      }
    }
  }
